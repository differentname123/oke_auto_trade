...
def beam_search_multi_k(profit_mat: np.ndarray,
    ...
    使用 Beam-Search 搜索候选组合，每个候选项保存：
      (候选策略索引元组, 最后一个策略索引, 指标字典, 累计收益数组, 累计开仓数组)
    """
    ...
    for current_k in range(2, max_k + 1):
        new_beam = []
        for candidate, last_idx, _, agg_profit, agg_kai in beam:
            for nxt in range(last_idx + 1, n_strategies):
                new_candidate = candidate + (nxt,)
                new_agg_profit = agg_profit + profit_mat[nxt]
                new_agg_kai = agg_kai + kai_mat[nxt]
                new_metrics = calc_metrics_from_agg(new_agg_profit, new_agg_kai, current_k, n_weeks)
                new_beam.append((new_candidate, nxt, new_metrics, new_agg_profit, new_agg_kai))
        if not new_beam:
            break
